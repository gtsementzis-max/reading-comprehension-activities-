...
%%CSSVARS%%
  ...
  *{box-sizing:border-box}
  ...
  .voc[data-def]{cursor:help}
  #vocTip{position:fixed;left:0;top:-9999px;z-index:999;max-width:230px;background:var(--c-dark);color:#fff;font-family:'Nunito',sans-serif;font-weight:700;font-size:.85rem;line-height:1.35;padding:8px 12px;border-radius:10px;box-shadow:var(--shadow);pointer-events:none;opacity:0;transform:translateY(-6px);transition:opacity .12s, transform .12s}
  #vocTip.show{opacity:1;transform:translateY(-10px)}
...
// Vocabulary hover/tap definitions: any word in MATCH gets a tooltip wherever
// it's highlighted in the passage. Works with mouse hover, keyboard focus, and tap.
(function(){
  const defMap = {};
  MATCH.forEach(m=>{ defMap[m.word.toLowerCase()] = m.def; });
  const tip = document.createElement('div');
  tip.id = 'vocTip';
  document.body.appendChild(tip);
  function showTip(el){
    const def = defMap[el.textContent.trim().toLowerCase()];
    if(!def) return;
    tip.textContent = def;
    const r = el.getBoundingClientRect();
    tip.style.left = Math.max(8, Math.min(window.innerWidth-238, r.left + r.width/2 - 115)) + 'px';
    tip.style.top = Math.max(8, r.top - 44) + 'px';
    tip.classList.add('show');
  }
  function hideTip(){ tip.classList.remove('show'); }
  document.querySelectorAll('.passage .voc').forEach(span=>{
    const def = defMap[span.textContent.trim().toLowerCase()];
    if(!def) return;
    span.dataset.def = def;
    span.tabIndex = 0;
    span.setAttribute('aria-label', span.textContent.trim()+': '+def);
    ...
    span.addEventListener('click', e=>{ e.stopPropagation(); tip.classList.contains('show') ? hideTip() : showTip(span); });
  });
...
{"activityId":"sentence-parts","projectKey":"sentence","title":"Subjects, Verbs &amp; Sentences","heroEmoji":"✍️","watermark":"📝","pageTitle":"Subjects, Verbs and Sentences","diagramFile":"diagrams/sentence-parts.svg",
 "win":"PERFECT, Amara! You build sentences like a pro! ✍️","cheer":"Subject, verb, done — brilliant work! 📝",
 "palette":{"primary":"#5e35b1","dark":"#4527a0","deep":"#311b92","accent":"#9575cd","accentSoft":"#ede7f6","cream":"#faf7ff","bgTop":"#f1ebfb","bgBottom":"#e6dcf7","glow1":"#5e35b122","glow2":"#311b9218"},
 "passageTitle":"How a Sentence Is Built",
 "passageHtml":"<p>Every sentence is a little machine with two working parts. The first part is the <span class=\"voc\">subject</span> — the person, animal, or thing the sentence is about. The second part tells what the subject does or is, and at its heart sits a <span class=\"voc\">verb</span>. Look at this example: <b>The tall girl</b> <b>rode</b> her bike to school. The subject is <i>The tall girl</i>, and the verb is <i>rode</i>. A quick trick is to say the subject and then ask \"…did what?\" The answer is almost always the verb.</p><p>Most verbs show action: <i>kicked</i>, <i>shouted</i>, <i>raced</i>. But in <i>The soup smells wonderful</i>, nothing is actually moving. There <i>smells</i> is a <span class=\"voc\">linking</span> verb, because it links the subject to a word that describes it. Sometimes two subjects or two verbs are joined by <i>and</i>, and we call that a <span class=\"voc\">compound</span> subject or a compound verb, as in <i>Amara and Dani baked a cake</i>.</p><p>Not every group of words is a <span class=\"voc\">sentence</span>. A <span class=\"voc\">phrase</span> is a group of words that travels together but does not contain a subject doing a verb — for example <i>under the wooden bridge</i>, <i>after the long storm</i>, or <i>a very sleepy cat</i>. Read one aloud and you can hear that something is missing: nobody is named, and nothing is happening, so it cannot stand on its own. Give a phrase a subject and a verb and it grows into a sentence at last: <i>The troll hid under the wooden bridge.</i> That is the test to remember — a real sentence needs a subject, a verb, and a complete thought.</p>",
 "questions":[{"type":"Main idea","q":"What is this passage mostly about?","opts":["why some verbs show action and others do not","how a subject, a verb, and a phrase fit together to build a sentence","the correct way to punctuate the end of a sentence"],"a":1},{"type":"Detail","q":"Every real sentence must have —","opts":["a subject, a verb, and a complete thought","at least one phrase in the middle","two or more describing words"],"a":0},{"type":"Vocabulary","q":"A \"phrase\" is —","opts":["a group of words with a subject and a verb in it","a group of words that travels together but has no subject doing a verb","any sentence that is missing its full stop"],"a":1},{"type":"Apply it","q":"In <i>The hungry seagull stole my sandwich</i>, which words are the subject?","opts":["stole my sandwich","The hungry seagull","my sandwich"],"a":1},{"type":"Apply it","q":"Which group of words is a phrase and NOT a sentence?","opts":["The soup smells wonderful.","Marco kicked the ball.","after the long storm"],"a":2},{"type":"Vocabulary","q":"In <i>The soup smells wonderful</i>, the word <i>smells</i> is —","opts":["a linking verb, because it links the subject to a describing word","an action verb, because something is moving","the subject of the sentence"],"a":0}],
 "match":[{"word":"subject","def":"the person, animal, or thing that a sentence is about","hint":"\"The first part is the <b>subject</b> — the person, animal, or thing the sentence is about.\""},{"word":"verb","def":"the word that shows the action or the state of being","hint":"\"The second part tells what the subject does or is, and at its heart sits a <b>verb</b>.\""},{"word":"phrase","def":"a group of words that travels together but has no subject doing a verb","hint":"\"A <b>phrase</b> is a group of words that travels together but does not contain a subject doing a verb.\""},{"word":"sentence","def":"a group of words with a subject, a verb, and a complete thought","hint":"\"Not every group of words is a <b>sentence</b>.\""},{"word":"linking","def":"describing a verb that joins the subject to a word that describes it instead of showing action","hint":"\"There <i>smells</i> is a <b>linking</b> verb, because it links the subject to a word that describes it.\""},{"word":"compound","def":"made of two parts joined together by a word such as <i>and</i>","hint":"\"…we call that a <b>compound</b> subject or a compound verb.\""}],
 "bank":["subject","verb","phrase","sentence","compound","linking","action","complete"],
 "fills":[{"text":"In <i>The tall girl rode her bike</i>, the words <i>The tall girl</i> are the ___.","a":"subject"},{"text":"In <i>Marco kicked the ball</i>, the word <i>kicked</i> is the ___.","a":"verb"},{"text":"<i>Under the wooden bridge</i> has no subject doing a verb, so it is only a ___.","a":"phrase"},{"text":"A group of words is a ___ only when it has a subject, a verb, and a complete thought.","a":"sentence"},{"text":"<i>Amara and Dani baked a cake</i> has two subjects joined by <i>and</i>, so it has a ___ subject.","a":"compound"},{"text":"In <i>The sky looks stormy</i>, the word <i>looks</i> is a ___ verb.","a":"linking"},{"text":"<i>Shouted</i>, <i>raced</i>, and <i>jumped</i> are all ___ verbs, because something is happening.","a":"action"},{"text":"The words <i>a very sleepy cat</i> do not express a ___ thought, so they cannot stand alone.","a":"complete","challenge":True}]},

{"activityId":"dragon-grammar","projectKey":"dragon","title":"Skarra the Dragon: Nouns, Verbs &amp; Articles","heroEmoji":"🐉","watermark":"🔥","pageTitle":"Skarra the Dragon","diagramFile":"diagrams/dragon-grammar.svg",
 "win":"PERFECT, Amara! Dragon-tamer! 🐉","cheer":"You breathed fire through that one! 🔥",
 "palette":{"primary":"#8e1f2f","dark":"#6e1624","deep":"#4a0f18","accent":"#d1737f","accentSoft":"#f9e3e6","cream":"#fff8f8","bgTop":"#fbeaec","bgBottom":"#f4d8dc","glow1":"#8e1f2f22","glow2":"#4a0f1818"},
 "passageTitle":"Skarra and the Empty River",
 "passageHtml":"<p>High above the village of Ardenmoor, in a smouldering hollow near the summit of the mountain, there lived a dragon named Skarra. Her home was an immense <span class=\"voc\">cavern</span> whose walls glittered with veins of copper, and at its centre lay her <span class=\"voc\">hoard</span> — a mound of coins, crowns, and stolen silver that no one had ever counted. Skarra was <span class=\"voc\">formidable</span>: her wings stretched wider than the village square, and a single breath could scorch a barley field to ash. Yet what the villagers dreaded most was not her fire but her patience. She was endlessly <span class=\"voc\">vigilant</span>, and in four hundred years not one thief had crept past her unnoticed.</p><p>One grey autumn morning, a girl named Elin climbed the <span class=\"voc\">treacherous</span> path to the cavern mouth. She carried no sword and no shield — only a bucket. The river below the mountain had run dry, she explained, because the dragon's constant fire had cracked the stone that fed the spring, and by winter the whole village would have nothing to drink. Skarra listened without blinking. For a long while she refused, insisting that the troubles of humans were no concern of hers, but Elin did not flinch, and at last the old dragon <span class=\"voc\">relented</span>. That night Skarra flew to the summit and, with three careful breaths, melted a channel through the ice so that the water ran again. She asked the village for nothing in return — though she did, quietly, keep the bucket.</p><p><b>Parts of speech in this story.</b> A <b>noun</b> names a person, place, animal, or thing: <i>dragon</i>, <i>village</i>, <i>cavern</i>, <i>bucket</i>. A <b>verb</b> shows an action or a state of being: <i>climbed</i>, <i>listened</i>, <i>melted</i>, <i>was</i>. An <b>article</b> is one of only three small words — <i>a</i>, <i>an</i>, <i>the</i> — and it always sits in front of a noun to point it out: <i>a bucket</i>, <i>an immense cavern</i>, <i>the old dragon</i>. Try the whole test on one line: <i>The dragon guarded a glittering hoard.</i> Here <i>The</i> and <i>a</i> are articles, <i>dragon</i> and <i>hoard</i> are nouns, and <i>guarded</i> is the verb.</p>",
 "questions":[{"type":"Main idea","q":"What is this story mostly about?","opts":["how a dragon builds her cavern inside a mountain","a dragon who is persuaded to help a village she has never cared about","a girl who tricks a dragon and steals part of her treasure"],"a":1},{"type":"Cause and effect","q":"Why had the village's river run dry?","opts":["thieves had blocked the spring while robbing the hoard","the dragon's constant fire had cracked the stone that fed the spring","the villagers had used all the water during a long summer"],"a":1},{"type":"Vocabulary","q":"\"Vigilant\" means —","opts":["always watchful and alert for danger","enormous in size and strength","willing to change your mind quickly"],"a":0},{"type":"Parts of speech","q":"In <i>Elin climbed the mountain</i>, which word is the verb?","opts":["mountain","Elin","climbed"],"a":2},{"type":"Parts of speech","q":"Which word in <i>Elin carried a heavy bucket</i> is an article?","opts":["a","carried","heavy"],"a":0},{"type":"Parts of speech","q":"Which group contains only nouns?","opts":["climbed, listened, melted","village, cavern, bucket","the, a, an"],"a":1}],
 "match":[{"word":"cavern","def":"a very large, deep cave","hint":"\"Her home was an immense <b>cavern</b> whose walls glittered with veins of copper.\""},{"word":"hoard","def":"a large store of treasure kept hidden away","hint":"\"…at its centre lay her <b>hoard</b> — a mound of coins, crowns, and stolen silver.\""},{"word":"formidable","def":"so powerful or impressive that it inspires fear","hint":"\"Skarra was <b>formidable</b>: her wings stretched wider than the village square.\""},{"word":"vigilant","def":"always watchful and alert for danger","hint":"\"She was endlessly <b>vigilant</b>, and in four hundred years not one thief had crept past her.\""},{"word":"treacherous","def":"full of hidden dangers and unsafe to cross","hint":"\"…a girl named Elin climbed the <b>treacherous</b> path to the cavern mouth.\""},{"word":"relented","def":"finally gave in after refusing for a long time","hint":"\"…Elin did not flinch, and at last the old dragon <b>relented</b>.\""}],
 "bank":["noun","verb","article","the","cavern","hoard","vigilant","relented"],
 "fills":[{"text":"A word that names a person, place, animal, or thing is a ___.","a":"noun"},{"text":"A word that shows an action or a state of being is a ___.","a":"verb"},{"text":"<i>A</i>, <i>an</i>, and <i>the</i> each sit in front of a noun, so each one is an ___.","a":"article"},{"text":"In the phrase <i>the smouldering hollow</i>, the article is the word ___.","a":"the"},{"text":"Skarra's glittering home deep inside the rock was an immense ___.","a":"cavern"},{"text":"The dragon slept beside a ___ of coins, crowns, and stolen silver.","a":"hoard"},{"text":"A guard who never once takes their eyes off the gate is ___.","a":"vigilant"},{"text":"After an hour of pleading, my father finally ___ and let us stay up late.","a":"relented","challenge":True}]}
